...
class _UploadRecordScreenState extends State<UploadRecordScreen>
  ...
  // ── NEW: stores OCR extracted text for AI explain ──
  String?        _extractedText;

  late AnimationController _checkController;
  late Animation<double>   _checkAnimation;

  List<Map<String, dynamic>> _existingEpisodes = [];
  ...
  Future<void> _loadEpisodes() async {
    ...
    try {
      ...
      final episodes = snapshot.docs
        ...
        return doctor != null && doctor.trim().isNotEmpty;
      })
          .map((doc) {
        final data     = doc.data();
        final dept     = data['department'] as String? ?? 'General Medicine';
        final deptInfo = _getDeptInfo(dept);

        final rawDate = data['date'];
        String date = '';
        if (rawDate is Timestamp) {
          date = _formatDate(rawDate.toDate());
        } else if (rawDate is String && rawDate.isNotEmpty) {
          try {
            date = _formatDate(DateTime.parse(rawDate));
          } catch (_) {
            date = rawDate;
          }
        }
        ...
        return {
          'id':         doc.id,
          'doctor':     data['doctor'] ?? 'Unknown Doctor',
          'hospital':   data['hospital'] ?? '',
          'department': dept,
          'date':       date,
          'color':      deptInfo['color'],
          'lightColor': (deptInfo['color'] as Color).withValues(alpha: 0.15),
          'icon':       deptInfo['icon'],
        };
      }).toList();

      // ── Deduplicate episodes by doctor+hospital ──
      final seen = <String>{};
      final uniqueEpisodes = episodes.where((ep) {
        final key = '${(ep['doctor'] as String).toLowerCase()}_${(ep['hospital'] as String).toLowerCase()}';
        return seen.add(key);
      }).toList();

      if (mounted) {
        setState(() {
          _existingEpisodes = uniqueEpisodes;
    ...
    final overlayEntry = OverlayEntry(
      builder: (context) => _TopBanner(
        ...
        message: customMessage ??
            (success ? 'Report uploaded successfully!' : 'Upload unsuccessful. Please try again.'),
      ),
    );
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          if (!mounted) return;
          _showTopBanner(success: false, customMessage: 'File too large. Maximum size is 10 MB.');
          return;
        }
        setState(() {
          _pickedFile          = file;
          ...
          _extractedText       = null; // reset extracted text on new file
        });
        _showTopBanner(success: true, customMessage: 'File selected: ${file.name}');

        final ext = file.name.split('.').last.toLowerCase();

        if (_uploadType == 1) {
          // ── Existing visit: run OCR silently for AI explain (no department detection needed) ──
          if (['jpg', 'jpeg', 'png'].contains(ext) && file.bytes != null) {
            _runOcrSilently(file.bytes!);
          }
          return;
        }

        // ── New visit & Direct upload: run full OCR + department detection ──
        if (['jpg', 'jpeg', 'png'].contains(ext) && file.bytes != null) {
      ...
      if (!mounted) return;
      _showTopBanner(success: false, customMessage: 'Could not open file picker. Please try again.');
    }
  }

  // ── Silent OCR for existing visits (only extracts text, no department UI) ──
  Future<void> _runOcrSilently(Uint8List bytes) async {
    try {
      final text = await VisionService.extractTextFromImage(bytes);
      if (mounted) {
        setState(() => _extractedText = text);
      }
    } catch (_) {
      // Silent fail — AI explain will just use title if OCR fails
    }
  }
  ...
  Future<void> _runOcrDetection(Uint8List bytes) async {
    ...
    try {
      // Save extracted text for AI explain
      _extractedText = await VisionService.extractTextFromImage(bytes);

      final detected = await VisionService.detectDepartmentFromText(
        _extractedText ?? '',
      );
      if (!mounted) return;
      if (detected != null) {
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Icon(deptInfo['icon'] as IconData, color: color, size: 30),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () { setState(() => _confirmedDepartment = detected); Navigator.pop(context); },
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                onPressed: () { Navigator.pop(context); _showDepartmentPicker(); },
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Select Department',
                  style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: _darkText)),
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return GestureDetector(
                      onTap: () => setSheetState(() => tempSelected = dept),
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 150),
                        margin: const EdgeInsets.only(bottom: 8),
                        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                        decoration: BoxDecoration(
                          color: isSel ? color.withValues(alpha: 0.08) : Colors.grey.shade50,
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                            color: isSel ? color : Colors.grey.withValues(alpha: 0.2),
                            width: isSel ? 1.5 : 1,
                          ),
                        ),
                        child: Row(
                          children: [
                            ...
                            Expanded(child: Text(dept,
                                style: TextStyle(color: isSel ? color : _darkText, fontSize: 14,
              ...
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  onPressed: tempSelected == null ? null : () {
                    setState(() => _confirmedDepartment = tempSelected);
                    Navigator.pop(ctx);
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _blue, foregroundColor: Colors.white,
  ...
  Map<String, dynamic> _getDeptInfo(String department) {
    ...
    return map[department] ?? {'icon': Icons.local_hospital_rounded, 'color': const Color(0xFF546E7A)};
  }
  ...
  Future<void> _submit() async {
    ...
    if (_uploadType == 0) {
      ...
      if (isValid && _selectedDate == null) { _showTopBanner(success: false, customMessage: 'Please select the visit date.'); return; }
      if (isValid && _pickedFile == null)   { _showTopBanner(success: false, customMessage: 'Please upload a file.'); return; }
    } else if (_uploadType == 1) {
      if (_selectedEpisodeId == null) { _showTopBanner(success: false, customMessage: 'Please select an existing episode.'); return; }
      isValid = _existingFormKey.currentState?.validate() ?? false;
      if (isValid && _pickedFile == null) { _showTopBanner(success: false, customMessage: 'Please upload a file.'); return; }
    } else {
      isValid = _directFormKey.currentState?.validate() ?? false;
      if (isValid && _selectedDate == null) { _showTopBanner(success: false, customMessage: 'Please select the report date.'); return; }
      if (isValid && _pickedFile == null)   { _showTopBanner(success: false, customMessage: 'Please upload a file.'); return; }
    }
    if (!isValid) return;

    // For new_visit and direct_upload only — existing_visit skips department check
    if (_uploadType != 1 && _confirmedDepartment == null && _pickedFile != null) {
      final ext = _pickedFile!.name.split('.').last.toLowerCase();
      if (['jpg', 'jpeg', 'png'].contains(ext) && _pickedFile!.bytes != null) {
        _showTopBanner(success: true, customMessage: 'Detecting department before upload...');
    ...
    setState(() { _isUploading = true; _uploadStatus = 'Uploading file to cloud...'; });
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) throw Exception('User not logged in');

      setState(() => _uploadStatus = 'Uploading file to cloud...');
      final s3Url = await S3Service.uploadFile(file: _pickedFile!, userId: user.uid, folder: 'records');

      setState(() => _uploadStatus = 'Saving record details...');
      final title = _uploadType == 2 ? _reportNameController.text.trim() : _titleController.text.trim();

      final selectedEpisode = _uploadType == 1
          ? _existingEpisodes.firstWhere((e) => e['id'] == _selectedEpisodeId)
          : null;

      final department = _uploadType == 1
          ? selectedEpisode!['department'] as String
          : (_confirmedDepartment ?? 'Pending AI Detection');
      ...
      final recordData = {
        ...
        'uploadType':    _uploadType == 0 ? 'new_visit' : _uploadType == 1 ? 'existing_visit' : 'direct_upload',
        'uploadedAt':    FieldValue.serverTimestamp(),
        'date':          _selectedDate != null ? Timestamp.fromDate(_selectedDate!) : FieldValue.serverTimestamp(),
        'userId':        user.uid,
        'department':    department,
        // ── Store extracted OCR text for AI explain ──
        ...
        if (_uploadType == 1) ...{
          ...
          'doctor':     selectedEpisode!['doctor'],
          'hospital':   selectedEpisode['hospital'],
          'department': selectedEpisode['department'],
        },
      };

      await FirebaseFirestore.instance
          .collection('users').doc(user.uid).collection('records').add(recordData);
  ...
  void _showSuccessSheet(String department) {
    final title    = _uploadType == 2 ? _reportNameController.text : _titleController.text;
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Record Uploaded!',
                style: TextStyle(color: _darkText, fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            ...
            Container(
              ...
              child: Row(children: [
                Icon(deptInfo['icon'] as IconData, color: color, size: 18),
                const SizedBox(width: 10),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Department', style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w600)),
                  Text(department,   style: TextStyle(color: color, fontSize: 14, fontWeight: FontWeight.bold)),
                ])),
            ...
            TextButton(
              ...
              child: const Text('Upload Another Record', style: TextStyle(color: _blue, fontSize: 14)),
            ),
          ],
        ),
      ),
  ...
  void _resetForm() {
    ...
    setState(() {
      _uploadType = 0; _selectedDate = null; _uploadedFileName = null;
      _pickedFile = null; _selectedEpisodeId = null;
      _confirmedDepartment = null; _extractedText = null;
    });
    _checkController.reset();
  }

  void _resetControllers() {
    _titleController.clear(); _doctorController.clear();
    _hospitalController.clear(); _reportNameController.clear();
    _selectedDate = null; _uploadedFileName = null;
    _pickedFile = null; _selectedEpisodeId = null;
    _confirmedDepartment = null; _extractedText = null;
  }
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildSectionLabel('What type of upload?', Icons.category_rounded),
                  const SizedBox(height: 12),
  ...
  Widget _buildAppBar() {
    return SliverAppBar(
      expandedHeight: 120, pinned: true, elevation: 0, backgroundColor: _blue,
      leading: IconButton(
            ...
            child: Padding(
              ...
              child: Row(children: [
                const Expanded(child: Column(
                  ...
                  children: [
                    Text('Upload Record', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 0.3)),
                    SizedBox(height: 4),
                    Text('Add a new document to your health vault', style: TextStyle(color: Colors.white70, fontSize: 13)),
                  ],
                )),
                Container(
        ...
        return Expanded(
          child: GestureDetector(
            onTap: () {
              ...
              if (index == 1) _loadEpisodes();
            },
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 200),
              margin: EdgeInsets.only(right: index < types.length - 1 ? 8 : 0),
              padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 8),
              decoration: BoxDecoration(
                color: isSelected ? _blue : Colors.white,
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: isSelected ? _blue : Colors.grey.withValues(alpha: 0.25), width: isSelected ? 2 : 1),
                boxShadow: isSelected
                    ? [BoxShadow(color: _blue.withValues(alpha: 0.25), blurRadius: 8, offset: const Offset(0, 3))]
                    : [BoxShadow(color: Colors.black.withValues(alpha: 0.04), blurRadius: 4, offset: const Offset(0, 2))],
              ),
              child: Column(children: [
                Icon(types[index]['icon'] as IconData, color: isSelected ? Colors.white : _blue, size: 22),
                const SizedBox(height: 6),
                Text(types[index]['label'] as String, textAlign: TextAlign.center,
                    style: TextStyle(color: isSelected ? Colors.white : _darkText, fontSize: 11, fontWeight: FontWeight.bold)),
                const SizedBox(height: 2),
                Text(types[index]['sub'] as String, textAlign: TextAlign.center,
                    style: TextStyle(color: isSelected ? Colors.white70 : Colors.grey.shade400, fontSize: 9)),
              ]),
            ),
          ),
        );
  ...
  Widget _buildNewVisitForm() {
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        _buildTextField(controller: _doctorController, hint: 'e.g. Dr. Raghav Menon',
        ...
        const SizedBox(height: 10),
        _buildTextField(controller: _hospitalController, hint: 'e.g. Apollo Hospital, Hyderabad',
        ...
        const SizedBox(height: 10),
        _buildTextField(controller: _titleController, hint: 'e.g. ECG Report, Blood Test',
            icon: Icons.description_outlined,
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Please enter report title' : null),
        const SizedBox(height: 20),
  ...
  Widget _buildExistingVisitForm() {
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildSectionLabel('Select Existing Episode', Icons.history_rounded),
        const SizedBox(height: 12),
        ...
        if (_loadingEpisodes)
          Container(
            ...
            child: const Center(child: Column(children: [
              ...
              SizedBox(height: 12),
              Text('Loading your episodes...', style: TextStyle(color: Colors.grey, fontSize: 13)),
            ])),
          )
        else if (_existingEpisodes.isEmpty)
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 4),
              const Text('Create a New Visit first to add reports\nto an existing episode.',
        ...
        _buildSectionLabel('Report Title', Icons.description_rounded),
        const SizedBox(height: 10),
        _buildTextField(controller: _titleController, hint: 'e.g. Follow-up Blood Test',
            icon: Icons.description_outlined,
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Please enter report title' : null),
        const SizedBox(height: 20),
        _buildSectionLabel('Upload File', Icons.attach_file_rounded),
  ...
  Widget _buildDirectUploadForm() {
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        _buildTextField(controller: _reportNameController, hint: 'e.g. Blood Test, Thyroid Report',
            icon: Icons.description_outlined,
            validator: (v) => (v == null || v.trim().isEmpty) ? 'Please enter report name' : null),
        const SizedBox(height: 20),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: isSelected ? _blue : Colors.grey.withValues(alpha: 0.2), width: isSelected ? 2 : 1),
        ...
        child: Row(children: [
          Container(
            ...
            child: Center(child: Icon(ep['icon'] as IconData, color: color, size: 18)),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(ep['doctor'] as String,
                style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: _darkText)),
            const SizedBox(height: 2),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: isSelected ? _blue : Colors.grey.shade300, width: 2),
            ),
            child: isSelected
      ...
      children: [
        GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: _uploadedFileName != null ? const Color(0xFFE8F5E9) : Colors.white,
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                color: _uploadedFileName != null ? const Color(0xFF2E7D32) : _blue.withValues(alpha: 0.4),
                width: 1.5,
              ),
            ),
            child: Column(children: [
              if (_isDetecting) ...[
                ...
                const SizedBox(height: 4),
                Text('AI is reading your report', style: TextStyle(fontSize: 11, color: Colors.grey.shade400)),
              ] else ...[
                Icon(
                  _uploadedFileName != null ? Icons.check_circle_rounded : Icons.attach_file_rounded,
                  color: _uploadedFileName != null ? const Color(0xFF2E7D32) : _blue, size: 32,
                ),
                const SizedBox(height: 8),
                Text(
                  _uploadedFileName != null ? 'File selected!' : 'Tap to upload file',
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold,
                      color: _uploadedFileName != null ? const Color(0xFF2E7D32) : _blue),
                ),
                const SizedBox(height: 4),
                Text(
                  _uploadedFileName != null
                      ? (_uploadedFileName!.length > 30 ? '${_uploadedFileName!.substring(0, 30)}...' : _uploadedFileName!)
                      : 'PDF, JPG, PNG supported  •  Max 10 MB',
                  style: TextStyle(fontSize: 11,
                      color: _uploadedFileName != null ? const Color(0xFF2E7D32) : Colors.grey.shade400),
                ),
                if (_uploadedFileName != null) ...[
                  ...
                  GestureDetector(
                    ...
                    child: const Text('Remove', style: TextStyle(color: Color(0xFFE53935), fontSize: 12, fontWeight: FontWeight.w600)),
                  ),
                ],
              ],
            ]),
        ...
        if (_confirmedDepartment != null && _uploadType != 1) ...[
          const SizedBox(height: 10),
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: (_getDeptInfo(_confirmedDepartment!)['color'] as Color).withValues(alpha: 0.08),
                borderRadius: BorderRadius.circular(30),
                border: Border.all(color: (_getDeptInfo(_confirmedDepartment!)['color'] as Color).withValues(alpha: 0.3)),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(_getDeptInfo(_confirmedDepartment!)['icon'] as IconData,
                    color: _getDeptInfo(_confirmedDepartment!)['color'] as Color, size: 14),
                const SizedBox(width: 6),
                Text(_confirmedDepartment!,
                    style: TextStyle(color: _getDeptInfo(_confirmedDepartment!)['color'] as Color,
                        fontSize: 12, fontWeight: FontWeight.bold)),
                const SizedBox(width: 6),
                Icon(Icons.edit_rounded, color: _getDeptInfo(_confirmedDepartment!)['color'] as Color, size: 12),
              ]),
            ),
          ),
        ],
  ...
  Widget _buildDatePicker(String hint) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFFF5F8FF), borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: _selectedDate != null ? _blue.withValues(alpha: 0.5) : _blue.withValues(alpha: 0.3),
        ...
        child: Row(children: [
          ...
          Expanded(child: Text(
            _selectedDate != null ? _formatDate(_selectedDate!) : hint,
            style: TextStyle(
              color: _selectedDate != null ? _darkText : Colors.grey.shade400, fontSize: 15,
              fontWeight: _selectedDate != null ? FontWeight.w600 : FontWeight.normal,
            ),
          )),
          Icon(
            _selectedDate != null ? Icons.check_circle_rounded : Icons.keyboard_arrow_down_rounded,
            color: _selectedDate != null ? _blue : Colors.grey, size: 20,
          ),
        ]),
      ),
    );
  ...
  Widget _buildSectionLabel(String text, IconData icon) {
    return Row(children: [
      ...
      Text(text, style: const TextStyle(color: _darkText, fontSize: 15, fontWeight: FontWeight.bold)),
    ]);
  }
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        border:            OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _blue.withValues(alpha: 0.3))),
        enabledBorder:     OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _blue.withValues(alpha: 0.3))),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isUploading
          ...
          const SizedBox(height: 8),
          Text(_uploadStatus, style: const TextStyle(fontSize: 12, color: Colors.white70)),
        ])
            : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.cloud_upload_rounded, size: 22),
          SizedBox(width: 10),
          Text('Upload Record', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        ]),
      ),
    );
  }
...
class _TopBannerState extends State<_TopBanner> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    final bgColor = widget.success ? const Color(0xFF2E7D32) : const Color(0xFFE53935);
    final icon    = widget.success ? Icons.check_circle_rounded : Icons.error_rounded;
    return Positioned(
      ...
      child: SlideTransition(
        ...
        child: Material(
          ...
          child: Container(
            width: double.infinity, color: bgColor,
            padding: EdgeInsets.only(top: MediaQuery.of(context).padding.top + 12, bottom: 14, left: 16, right: 16),
            child: Row(children: [
